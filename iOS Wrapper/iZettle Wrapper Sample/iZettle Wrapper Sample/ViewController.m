...
@implementation ViewController {
    iZettleURLSchemePaymentInfo *_lastPaymentInfo;
...
- (IBAction)charge:(id)sender {
    ...
    [[iZettleURLScheme shared] chargeAmount:amount currency:nil reference:_lastReference completion:^(iZettleURLSchemePaymentInfo *paymentInfo, NSError *error) {
...
- (IBAction)openIZettle:(id)sender {
    [[iZettleURLScheme shared] openIZettleApp];
}

- (IBAction)lastPaymentInfo:(id)sender {
    [[iZettleURLScheme shared] retrievePaymentInfoForReference:_lastReference completion:^(iZettleURLSchemePaymentInfo *paymentInfo, NSError *error) {
        _lastPaymentInfo = paymentInfo;
        _lastError = error;
        [self _showConsole];
    }];
...
- (IBAction)refundLastPayment:(id)sender {
    ...
    _lastReference = [self _uniqueReference];
    [[iZettleURLScheme shared] refundPaymentWithReference:paymentReference refundReference:_lastReference completion:^(iZettleURLSchemePaymentInfo *paymentInfo, NSError *error) {
        _lastPaymentInfo = paymentInfo;
        _lastError = error;
        [self _showConsole];
    }];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString: @"showConsole"] && sender == self) {
        ConsoleViewController *console = (ConsoleViewController *)segue.destinationViewController;
        if (_lastError) {
